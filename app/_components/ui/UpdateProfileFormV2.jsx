...
function UpdateProfileForm({ user }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setFormError({ comune: "", cap: "" }); // reset errori

    const formData = new FormData(e.target);
    const comuneInput = formData.get("comune")?.trim().toLowerCase();
    const capInput = formData.get("cap")?.trim();

    const comuneObj = comuni.find(
      (c) => c.denominazione_ita.toLowerCase() === comuneInput
    );

    const capObj = comuni.find(
      (c) =>
        c.denominazione_ita.toLowerCase() === comuneInput && c.cap === capInput
    );

    if (!comuneObj) {
      setFormError((prev) => ({ ...prev, comune: "Comune non valido" }));
      return;
    }
    if (!capObj) {
      setFormError((prev) => ({ ...prev, cap: "Cap non valido" }));
      return;
    }
