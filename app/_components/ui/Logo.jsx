...
function Logo({ complete = false, isFooter = false }) {
  return (
    ...
    >
      ...
      <span
        className={`${isFooter ? "" : "_hidden"} text-3xl font-bold text-shadow-2xs md:inline dark:opacity-90`}
        aria-hidden="true"
      >
        V
      </span>
      {complete && (
        <span
          className={`${isFooter ? "" : "_hidden"} text-2xl font-bold tracking-wider text-shadow-2xs md:inline dark:opacity-90`}
          aria-hidden="true"
        >
          esugusto
        </span>
