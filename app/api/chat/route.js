...
export async function POST(request) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("Errore Gemini:", err);
      return new Response(
        JSON.stringify({ error: "Errore dalla Gemini API" }),
        {
          status: 500,
        }
      );
    ...
    // Trova la prima parte con una stringa
    const firstTextPart = data.candidates?.[0]?.content?.parts?.find(
      (p) => typeof p.text === "string"
    )?.text;

    const reply = firstTextPart ?? "Nessuna risposta generata.";
    ...
    console.error("Errore fetch Gemini:", error);
